...
export async function del(index: string, type: string, query) {
    await axios({
        method: 'POST',
        url: `${esUrl}/${index}/${type}/_delete_by_query`,
        headers: { 'content-type': 'application/json' },
        data: query,
    })
}
export async function search(_index, _type, query) {
    let url = `${esUrl}/${_index}/${_type}/_search`
    return await axios({
        method: 'POST',
        url,
        data: query,
...
export async function loadJsonFromDb(_index, _type, size, query = null, sort = null) {
    let url = esUrl + '/' + _index + '/' + _type + '/_search?size=' + size
    if (sort) {
        url = url + '&sort=' + encodeURI(sort)
    }
    if (query) {
        url = url + '&q=' + encodeURI(query.key) + ':"' + urlencode(query.value) + '"'
    }
...
if (require.main === module) {
    ;(async function() {
        const attendanceEntries = await loadJsonFromDb('attendance', 'attendance', 100)
